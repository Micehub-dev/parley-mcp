...
1. 코어: `multi-llm-parley` 로컬 MCP 서버
...
- 세션을 다시 이어갈 때도 `parleySessionId` 와 서버 저장 상태만 있으면 된다.
...
- `parley_step` 호출 시 `expectedStateVersion` 을 같이 보낸다.
- 서버는 버전이 맞을 때만 갱신한다.
...
- `parley://workspaces`
- `parley://workspace/{workspaceId}/topics`
- `parley://workspace/{workspaceId}/topic/{topicId}`
- `parley://workspace/{workspaceId}/topic/{topicId}/threads`
- `parley://workspace/{workspaceId}/session/{sessionId}/summary`
...
- `parley.review_topic`
- `parley.start_from_topic`
- `parley.challenge_decision`
